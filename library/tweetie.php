...
class MyTwitterWidget extends WP_Widget {
	...
	function MyTwitterWidget() {
		parent::WP_Widget(
			false, __( 'My Twitter Widget', 'bonestheme' ),
			array('description' => __( 'Displays a list of tweets from a specified user name', 'bonestheme' )),
			array('width' => '400px')
		);
	}
	/** The back-end form **/
...
<p>
  <label for='<?php echo $this->get_field_id( 'limit' ); ?>'>
    <?php _e( 'Tweets to show:', 'bonestheme' ); ?>
  ...
  <label for='<?php echo $this->get_field_id( 'limit' ); ?>'>
    <?php _e( 'Screenname to show:', 'bonestheme' ); ?>
...
<?
	...
	function widget( $args, $instance ) {
		...
		$tweets = getTweets('susummercollege', '3');
...
foreach($tweets as $tweet){
    ...
    if($tweet['text']){
        ...
        */
        ...
        echo '
        <p class="timestamp">
            <a href="https://twitter.com/susummercollege/status/'.$tweet['id_str'].'" target="_blank">
        ...
        <a href="http://twitter.com/susummercollege" target="_blank" class="btn btn-default btn-block">SUmmer College Twitter feed <i class="fa fa-arrow-right"></i></a>';
...
echo '<h3>Summer College Twitter Feed</h3>';
echo '<ul class="unstyled twitter-feed clearfix">';
foreach($tweets as $tweet){
		...
		if($tweet['text']){
			...
			*/
			...
			// get the sprite or images from twitter's developers resource and update your stylesheet
			echo '
			<ul class="list-inline twitter_intents clearfix">
				<li><a class="reply" href="https://twitter.com/intent/tweet?in_reply_to='.$tweet['id_str'].'"><i class="icon-reply"></i></a></li>
				<li><a class="retweet" href="https://twitter.com/intent/retweet?tweet_id='.$tweet['id_str'].'"><i class="icon-mail-forward"></i></a></li>
				<li><a class="favorite" href="https://twitter.com/intent/favorite?tweet_id='.$tweet['id_str'].'"><i class="icon-star"></i></a></li>
			</ul>';
